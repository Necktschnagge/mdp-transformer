...
void check_valid_mdp_and_load_mdp_from_json(const nlohmann::json& input, mdp& fill_in) { // do check
	// do all checks
	/*
	states:
	* is list
	...
	*/
	...
	// fill up probabilities with states without probabilities:
	for (const auto& s : fill_in.states) {
		fill_in.probabilities[s]; // inserts an empty map if not already there.
	}
	...
	// fill up missing reward entries with zeros...
	for (const auto& state_paired_transitions : fill_in.probabilities) {
		for (const auto& action_paired_distr : state_paired_transitions.second) {
			const auto& state{ state_paired_transitions.first };
			const auto& action{ action_paired_distr.first };
			fill_in.rewards[state][action] = rational_type(0);
		}
	}
	// rewards
